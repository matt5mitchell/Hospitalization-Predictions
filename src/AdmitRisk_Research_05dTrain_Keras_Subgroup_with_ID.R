...
## Subgroup model WITH subject-specific treatment (patient ID as input)
...
nn_fit <- function(units_gru1, units_gru2, units_d1, units_d2, dropout, l2, lr, epoch, train_list, test_list, ...) {
  ...
  subject_input_shape <- as.numeric(dim(test_list[[3]])[2]) #2D input
  ...
  dense_input <- layer_input(shape = dense_input_shape,
                             name = 'dense_input')
  subject_input <- layer_input(shape = subject_input_shape,
                               name = 'subject_input')
  ...
  subgroup_out <- dense_input %>%
    subgroup_model #weights previously frozen
  
  dense_1_out <- layer_concatenate(c(gru_out, subgroup_out)) %>%
  ...
  main_output <- layer_concatenate(c(dense_1_out, subject_input)) %>%
    layer_dropout(rate = dropout) %>%
    layer_dense(
      units = units_d2, 
      activation = 'relu', 
  ...
  model <- keras_model(
    inputs=list(gru_input, dense_input, subject_input),
  ...
  #Fit the model
  model %>% fit(
    x = list(gru_input = train_list[[1]], dense_input = train_list[[2]], subject_input = train_list[[3]]), 
    y = list(main_output = train_list[[4]], aux_output = train_list[[5]]),
    validation_data = list(list(gru_input = test_list[[1]], dense_input = test_list[[2]], subject_input = test_list[[3]]),
  ...
  eval <- model %>% evaluate(list(gru_input = test_list[[1]], dense_input = test_list[[2]], subject_input = test_list[[3]]),
                             list(main_output = test_list[[4]], aux_output = test_list[[5]]))
  
  #Predictions with held out test data
  pred <- predict(model, list(gru_input = test_list[[1]], dense_input = test_list[[2]], subject_input = test_list[[3]]))[[2]]
